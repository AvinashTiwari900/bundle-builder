    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {AI_QUICK_ACTIONS.map((p, idx) => (
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-white border border-slate-200 text-slate-700 text-xs font-semibold hover:border-blue-400 hover:bg-blue-50/70 hover:text-blue-700 transition-all shadow-xs cursor-pointer text-left"
              >
                {p.label}
              </button>
            ))}
          ...
          {/* Chat Messages Card */}
          <div className="bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm h-[580px] flex flex-col justify-between">
                      ...
                      {m.card && (
                        <div className="mt-3 pt-3 border-t border-slate-200 space-y-2.5">
                          ...
                          {m.card.type === 'navigation' && (
                            <div className="p-3 bg-blue-50/90 rounded-xl border border-blue-200 flex items-center justify-between gap-3">
                              <div>
                            ...
                            <div className="p-4 bg-white rounded-2xl border border-slate-200 space-y-3">
                              ...
                              <div className="space-y-2">
                                {m.card.data.map((step: any) => (
                                  <div
                                    key={step.step}
                                    className="p-2.5 bg-slate-50 rounded-xl border border-slate-200/80 flex items-center justify-between text-xs hover:border-blue-300 transition-colors"
                                  >
                        ...
                        <div className="mt-3 pt-2.5 border-t border-slate-200 flex flex-wrap gap-1.5">
                          {m.quickActions.map((qa, i) => (
                            <button
                              ...
                              className="px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-[11px] font-bold border border-blue-200 transition-colors cursor-pointer"
                            >
                              {qa.label}
                            </button>
                          ))}
            ...
            <div className="pt-4 border-t border-slate-100 flex items-center gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white transition-all"
              />
              <Button
                variant="primary"
                size="md"
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div className="p-3 bg-slate-50 rounded-2xl flex items-center justify-between">
                <span className="text-slate-600 font-medium">Resume ATS Match</span>
              ...
              <div className="p-3 bg-slate-50 rounded-2xl flex items-center justify-between">
                <span className="text-slate-600 font-medium">Active Applications</span>
                <span className="font-extrabold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg border border-blue-200">
                  {applications.length} Active
                </span>
              </div>

              <div className="p-3 bg-slate-50 rounded-2xl flex items-center justify-between">
                <span className="text-slate-600 font-medium">Portfolio Strength</span>
                <span className="font-extrabold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-lg border border-purple-200">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm space-y-3">
            <h3 className="font-extrabold text-sm text-slate-900">Guided Workflows</h3>

            <div className="space-y-2">
              <button
                type="button"
                onClick={() => handleSend('How do I create my portfolio?')}
                className="w-full text-left p-3 rounded-2xl bg-blue-50/60 hover:bg-blue-100 border border-blue-100 flex items-center justify-between transition-colors cursor-pointer"
              >
              ...
              <button
                ...
                className="w-full text-left p-3 rounded-2xl bg-indigo-50/60 hover:bg-indigo-100 border border-indigo-100 flex items-center justify-between transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-2.5 text-xs font-bold text-slate-800">
              ...
              <button
                ...
                className="w-full text-left p-3 rounded-2xl bg-purple-50/60 hover:bg-purple-100 border border-purple-100 flex items-center justify-between transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-2.5 text-xs font-bold text-slate-800">
                  <Video size={16} className="text-purple-600 shrink-0" />
                  <span>Interview Preparation Plan</span>
                </div>
                <ChevronRight size={15} className="text-purple-500" />
              </button>

              <button
                type="button"
                onClick={() => handleSend('Where can I upload my documents?')}
                className="w-full text-left p-3 rounded-2xl bg-emerald-50/60 hover:bg-emerald-100 border border-emerald-100 flex items-center justify-between transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-2.5 text-xs font-bold text-slate-800">
